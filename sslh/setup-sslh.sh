...
##########################
cat <<EOF > /etc/systemd/system/sslh8443.service
[Unit]
Description=SSL/SSH multiplexer
After=network.target

[Service]
ExecStart=/usr/sbin/sslh --foreground --user sslh --listen 0.0.0.0:8443 --ssh 127.0.0.1:23 --openvpn 127.0.0.1:995 --http 127.0.0.1:2082 --pidfile /var/run/sslh/sslh443.pid
KillMode=process
...
systemctl daemon-reload
systemctl enable sslh8443
systemctl start sslh8443
##########################
