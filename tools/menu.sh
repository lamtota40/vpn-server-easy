...
cpuinfo=$(cat /proc/cpuinfo | grep MHz | awk '{print $4}' | sort | uniq -c | awk '{printf "%dx %sMHz,", $1, $2}' | sed 's/,$//')
...
echo -e "${PINK}CPU${NC}	 :${BLUE} Usage = ${YEL}"$cpuuse"% ${BLUE}| C.Speed = ${YEL}"$cpuinfo" ${NC}"
...
9)
	...
	echo "2. Speedtest"
	echo "3. Benchmark"
	echo "4. Scan Open Port"
	echo "5. Monitoring RAM"
