...
NC='\033[0m'
clear
gmt_info=$(timedatectl | grep "Local time:" | awk '{ print $6 }')
...
echo -e "\e[36m╘════════════════════════════════════════════╛\033[0m"
echo -e "${RED}Time${NC}	 :" `date "+%H:%M:%S"` "|" `date "+%d/%m/%y"` "| GMT $gmt_info"
echo -e "${RED}Uptime${NC}	 :" `uptime -p`
echo -e "${RED}Host${NC}	 : $domain | $(jq -r '.query' <<< "$cekip")"
echo -e "${RED}NS Host${NC}	 : $nsdomain"
echo -e "${RED}ISP${NC}	 : $(jq -r '.isp' <<< "$cekip") | $(jq -r '.country' <<< "$cekip")"
echo -e "${RED}RAM${NC}	 : Usage = "$ram_usage"Mb ($(($ram_usage * 100 / $ram_total))%) | Total = "$ram_total"Mb"
echo -e "${RED}SWAP RAM${NC} : Usage = "$swap_usage"Mb ("$swap_percent"%) | Total = "$swap_total"Mb"
echo -e "${RED}HDD${NC}      : Usage = $hdd_usage  ($(awk '{print $5}' <<< "$hddinfo")) | Total = $hdd_total"
echo -e "\e[36m╒════════════════════════════════════════════╕\033[0m"
echo -e " Daily Data Usage   :  ${YEL}$daily_usage${NC}"
echo -e " Monthly Data Usage :  ${YEL}$monthly_usage${NC}"
echo -e "\e[36m╘════════════════════════════════════════════╛\033[0m"
echo -e " \E[41;1;37m              Version Script 2.0            \E[0m"
echo -e " \E[44;1;37m Install : 14/05/23      Exp : Lifetime     \E[0m"
echo -e "\e[36m╒════════════════════════════════════════════╕\033[0m"
echo -e " \E[46;1;37m                    MENU                    \E[0m"
echo -e "\e[36m╘════════════════════════════════════════════╛\033[0m"
...
echo -e "8. ${YEL}Info${NC}"
echo -e "9. ${YEL}Tools${NC}"
echo -e "0. ${YEL}Exit Menu${NC}"
echo -e "\e[36m═════════════════════════════════════════════\033[0m"
read -p "Choose Options [0-9] : " num
while ! [[ "${num}" =~ ^[0-9]$ ]]; do
echo "Wrong your input!"
read -p "Choose Options [0-9] : " num
done
...
1)
	clear
	echo "1. Add User for SSH+OpenVPN+Socks5+UDP"
	echo "2. Add User for V2ray(Vmess/Vless/Tojan/shadowsocks)"
	echo "3. Add User for All in one(SSH+UDP+V2ray)"
	echo "0. Back to Menu"
	read -p "Choose Options [0-4] :" num1
	case $num1 in
	1)
	clear
	/root/myvpn/addusers
