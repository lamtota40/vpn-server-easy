...
#instalasi Websocket (accept http port 80 forward to port 23[dropbear])
wget -O /usr/local/bin/ws-http80 https://raw.githubusercontent.com/lamtota40/vpn-server-easy/main/websocket/ws-http80.py && chmod +x /usr/local/bin/ws-http80

cat > /etc/systemd/system/ws-http80.service <<-END
[Unit]
Description=HTTP SSH Over Websocket 80 Python
...
Restart=on-failure
ExecStart=/usr/bin/python -O /usr/local/bin/ws-http80

[Install]
WantedBy=multi-user.target
END
chmod +x /etc/systemd/system/ws-http80.service
###################################################
...
systemctl enable ws-http80.service
systemctl start ws-http80.service
systemctl restart ws-http80.service
